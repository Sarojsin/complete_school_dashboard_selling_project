...
class NoticeRepository:
    ...
    async def get_by_id(db: AsyncSession, notice_id: int) -> Optional[Notice]:
        result = await db.execute(
            select(Notice).filter(Notice.id == notice_id)
        )
    ...
    async def get_all(db: AsyncSession, skip: int = 0, limit: int = 100,
                target_role: str = None, priority: str = None,
                target_grade: str = None) -> List[Notice]:
        query = select(Notice)
            ...
            if target_grade:
                query = query.filter(
                    or_(
                        ...
                        and_(Notice.target_role == target_role, or_(Notice.target_grade == target_grade, Notice.target_grade.is_(None)))
                    )
                )
            else:
                query = query.filter(
    ...
    @staticmethod
    async def get_active_notices(db: AsyncSession, target_role: str = None, target_grade: str = None) -> List[Notice]:
        """Get all active (non-expired) notices"""
        return await NoticeRepository.get_all(db, target_role=target_role, target_grade=target_grade)

    @staticmethod
    async def get_by_priority(db: AsyncSession, priority: str,
                       target_role: str = None, target_grade: str = None) -> List[Notice]:
        """Get notices by priority level"""
        return await NoticeRepository.get_all(db, priority=priority, target_role=target_role, target_grade=target_grade)

    @staticmethod
    async def get_urgent_notices(db: AsyncSession, target_role: str = None, target_grade: str = None) -> List[Notice]:
        """Get urgent notices"""
        return await NoticeRepository.get_by_priority(db, 'urgent', target_role, target_grade)
    ...
    async def get_recent_notices(db: AsyncSession, days: int = 7,
        ...
        query = select(Notice).filter(Notice.created_at >= cutoff_date)
    ...
    async def search_notices(db: AsyncSession, query: str, target_role: str = None) -> List[Notice]:
        ...
        search_query = select(Notice).filter(
            or_(
