...
class AccountSectionService:
    ...
    async def bulk_create_fees(self, fees_data: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        for fee_data in fees_data:
            try:
                fee = await self.repository.create_fee(fee_data)
                created_fees.append({
                    "id": fee.id,
                    "student_id": fee.student_id,
                    "fee_type": fee.fee_type,
                ...
                errors.append(f"Error creating fee for student {fee_data.get('student_id')}: {str(e)}")
