...
from modules.school.school_courses.models import SchoolCourse as Course, CourseEnrollment
...
class CourseRepository:
    ...
    async def get_enrolled_students(db: AsyncSession, course_id: int) -> List[Student]:
        result = await db.execute(
            select(Student).join(CourseEnrollment).options(
    ...
    @staticmethod
    async def enroll_student(db: AsyncSession, course_id: int, student_id: int):
    ...
    @staticmethod
    async def remove_student(db: AsyncSession, course_id: int, student_id: int):
        stmt = delete(CourseEnrollment).filter(
            CourseEnrollment.course_id == course_id,
