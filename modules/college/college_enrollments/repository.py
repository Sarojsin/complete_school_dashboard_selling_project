...
class EnrollmentRepository:
    ...
    async def create(self, data: EnrollmentCreate) -> EnrollmentModel:
        ...
        enrollment = EnrollmentModel(**data.model_dump())
        self.db.add(enrollment)
        await self.db.commit()
        await self.db.refresh(enrollment)
        return enrollment

    async def get(self, enrollment_id: int) -> Optional[EnrollmentModel]:
    ...
    async def get_by_student_course_semester(
        ...
        if semester_id is not None:
            query = query.where(EnrollmentModel.semester_id == semester_id)
        result = await self.db.execute(query)
        return result.scalar_one_or_none()

    async def get_by_student(self, student_id: int) -> List[EnrollmentModel]:
        """Get all enrollments for a student"""
        result = await self.db.execute(
            select(EnrollmentModel)
            .where(EnrollmentModel.student_id == student_id)
            .order_by(EnrollmentModel.enrollment_date.desc())
        )
        return list(result.scalars().all())

    async def get_by_course(self, course_id: int) -> List[EnrollmentModel]:
    ...
    async def get_all(
        ...
        query = query.order_by(EnrollmentModel.enrollment_date.desc()).offset(skip).limit(limit)
        result = await self.db.execute(query)
        return list(result.scalars().all())

    async def update(self, enrollment_id: int, data: EnrollmentUpdate) -> Optional[EnrollmentModel]:
    ...
    async def count(self, course_id: Optional[int] = None, semester_id: Optional[int] = None) -> int:
        """Count enrollments"""
        query = select(func.count(EnrollmentModel.id))
        if course_id is not None:
            query = query.where(EnrollmentModel.course_id == course_id)
        if semester_id is not None:
            query = query.where(EnrollmentModel.semester_id == semester_id)
