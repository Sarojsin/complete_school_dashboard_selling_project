...
CREATE TABLE notices (
    ...
    notice_type VARCHAR(50) DEFAULT 'general' CHECK (notice_type IN ('general', 'academic', 'event', 'circular', 'alert', 'holiday')),
...
CREATE TABLE school_book_loans (
  ...
  CONSTRAINT chk_loan_due_future CHECK (due_date >= borrowed_at::date),
  CONSTRAINT chk_return_after_issue CHECK (actual_return_date IS NULL OR actual_return_date >= borrowed_at::date)
);
...
CREATE UNIQUE INDEX uk_payment_transaction ON school_payments(transaction_id) WHERE transaction_id IS NOT NULL
