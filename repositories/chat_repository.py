...
from models.models import User, Parent, Student, CourseEnrollment, Course, Teacher
...
class ChatRepository:
    ...
    async def get_parent_teachers(db: AsyncSession, parent_id: int) -> List[dict]:
        ...
        t_res = await db.execute(
            select(Teacher).join(Course).filter(Course.id.in_(course_ids)).distinct()
        )
        ...
        for teacher in teachers:
            # Need user for teaching
            u_res = await db.execute(select(User).filter(User.id == teacher.user_id))
    ...
    async def get_teacher_parents(db: AsyncSession, teacher_id: int) -> List[dict]:
        ...
        p_res = await db.execute(
            select(Parent).join(Student).filter(
        ...
        for parent in parents:
            # Need user for parent
            u_res = await db.execute(select(User).filter(User.id == parent.user_id))
            user_obj = u_res.scalars().first()
    ...
    async def get_all_teachers(db: AsyncSession, parent_id: int) -> List[dict]:
        ...
        # Get all teachers
        t_res = await db.execute(select(Teacher))
        teachers = t_res.scalars().all()
        
        # Format result
        final_result = []
        for teacher in teachers:
            # Need user for teacher
            u_res = await db.execute(select(User).filter(User.id == teacher.user_id))
            user_obj = u_res.scalars().first()
