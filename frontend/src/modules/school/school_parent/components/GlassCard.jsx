...
// Child Card - Shows child info with quick stats
export const ChildCard = ({ child, onClick, delay = 0 }) => {
  const statusColors = { active: 'text-emerald-400', inactive: 'text-slate-400' };
  return (
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay }}
      onClick={onClick} whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }}
      className="bg-gradient-to-br from-slate-800/80 to-slate-900/80 backdrop-blur-xl border border-white/10 shadow-xl rounded-2xl p-6 cursor-pointer hover:border-white/30 transition-all">
      <div className="flex items-center gap-4 mb-4">
        <div className="h-16 w-16 rounded-full bg-gradient-to-br from-primary-500/30 to-purple-500/30 flex items-center justify-center text-2xl font-bold text-white">
          {child?.name?.charAt(0) || 'S'}
        </div>
        <div>
          <h3 className="text-lg font-semibold text-white">{child?.name || 'Student'}</h3>
          <p className="text-white/60 text-sm">{child?.class || 'Class'} - {child?.section || 'Section'}</p>
        </div>
      </div>
      <div className="grid grid-cols-3 gap-3">
        <div className="text-center p-2 rounded-lg bg-white/5">
          <p className="text-white/50 text-xs">Attendance</p>
          <p className="text-emerald-400 font-bold">{child?.attendance || '95%'}</p>
        </div>
        <div className="text-center p-2 rounded-lg bg-white/5">
          <p className="text-white/50 text-xs">GPA</p>
          <p className="text-primary-400 font-bold">{child?.gpa || '3.5'}</p>
        </div>
        <div className="text-center p-2 rounded-lg bg-white/5">
          <p className="text-white/50 text-xs">Status</p>
          <p className={`font-bold ${statusColors[child?.status] || statusColors.active}`}>{child?.status || 'Active'}</p>
        </div>
      </div>
    </motion.div>
  );
...
export const GradeBadge = ({ grade }) => {
  const getGradeColor = (g) => {
    if (g >= 90) return 'bg-emerald-500/20 text-emerald-400 border-emerald-500/30';
    if (g >= 80) return 'bg-primary-500/20 text-primary-400 border-primary-500/30';
    if (g >= 70) return 'bg-amber-500/20 text-amber-400 border-amber-500/30';
    if (g >= 60) return 'bg-orange-500/20 text-orange-400 border-orange-500/30';
    return 'bg-red-500/20 text-red-400 border-red-500/30';
  };
  return <span className={`px-3 py-1 rounded-full text-xs font-medium border ${getGradeColor(grade)}`}>{grade}</span>;
};

// Attendance Badge
export const AttendanceBadge = ({ status }) => {
  const styles = { present: 'bg-emerald-500/20 text-emerald-400 border-emerald-500/30', absent: 'bg-red-500/20 text-red-400 border-red-500/30', late: 'bg-amber-500/20 text-amber-400 border-amber-500/30', leave: 'bg-primary-500/20 text-primary-400 border-primary-500/30' };
  return <span className={`px-3 py-1 rounded-full text-xs font-medium border ${styles[status] || styles.present}`}>{status?.toUpperCase()}</span>;
};
...
export const EmptyState = ({ icon: Icon, title, description, action }) => (
  <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="glass-card p-12 text-center">
    {Icon && <div className="w-24 h-24 mx-auto mb-6 rounded-full bg-gradient-to-br from-amber-500/30 to-orange-500/30 flex items-center justify-center"><Icon className="w-12 h-12 text-white/60" /></div>}
    <h3 className="text-xl font-semibold text-white mb-2">{title}</h3>
    {description && <p className="text-white/50 max-w-sm mx-auto mb-6">{description}</p>}
    {action && <button className="bg-amber-600 hover:bg-amber-500 text-white font-medium py-2 px-6 rounded-xl transition-colors">{action}</button>}
  </motion.div>
);
...
export const SearchInput = ({ value, onChange, placeholder = 'Search...', className = '' }) => (
  <div className={`relative ${className}`}>
    <input type="text" value={value} onChange={onChange} placeholder={placeholder}
      className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-2 pl-10 text-white placeholder-white/40 focus:outline-none focus:border-amber-500/50 transition-colors" />
...
// Progress Bar
export const ProgressBar = ({ value, max = 100, label, color = 'primary', showPercentage = true }) => {
...
export const Badge = ({ children, variant = 'default' }) => {
  const variants = { default: 'bg-white/10 text-white/60', primary: 'bg-primary-500/20 text-primary-400', amber: 'bg-amber-500/20 text-amber-400', success: 'bg-emerald-500/20 text-emerald-400', warning: 'bg-amber-500/20 text-amber-400', danger: 'bg-red-500/20 text-red-400' };
  return <span className={`inline-flex items-center rounded-full font-medium ${variants[variant]} px-3 py-1 text-sm`}>{children}</span>;
};

export default { GlassCard, ChildCard, StatCard, GradeBadge, AttendanceBadge, FeeStatusBadge, DataTable, GlassButton, EmptyState, SearchInput, Modal, ProgressBar, Badge };
