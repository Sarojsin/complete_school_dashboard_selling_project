...
from backup.schemas.group_post import GroupPostCreate, GroupPostUpdate
...
async def list_posts(
    ...
    try:
        ...
        return RedirectResponse(url=f"/{role}/groups/{group_id}", status_code=303)
...
async def create_post(
    ...
    try:
        ...
        role = current_user.role.value.lower()
        return RedirectResponse(
            url=f"/{role}/groups/{group_id}/posts/create",
            status_code=303
        )
...
async def delete_post(
    ...
    role = current_user.role.value.lower()
    return RedirectResponse(
        url=f"/{role}/groups/{group_id}/posts",
        status_code=303
    )
