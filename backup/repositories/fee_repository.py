...
from backup.models.models import FeeRecord, Student, User
...
class FeeRepository:
    ...
    @staticmethod
    async def get_all(db: AsyncSession, limit: int = 50, offset: int = 0) -> List[FeeRecord]:
        result = await db.execute(
            select(FeeRecord).options(
                joinedload(FeeRecord.student).joinedload(Student.user)
            ).order_by(desc(FeeRecord.id)).limit(limit).offset(offset)
        )
        return result.scalars().unique().all()
