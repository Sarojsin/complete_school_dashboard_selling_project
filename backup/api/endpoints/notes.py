...
from backup.schemas.misc import NoteResponse
...
async def upload_note(
    ...
    note = await NotesRepository.create(db, note_data)
...
async def get_my_notes(
    ...
    notes = await NotesRepository.get_by_teacher(db, teacher.id)
    return notes
...
async def delete_note(
    ...
    note = await NotesRepository.get_by_id(db, note_id)
    if not note:
    ...
    await NotesRepository.delete(db, note)
...
async def get_course_notes(
    ...
    notes = await NotesRepository.get_by_course(db, course_id)
    return notes
...
async def search_notes(
    ...
    """Search notes"""
    notes = await NotesRepository.search_notes(db, query, course_id)
...
async def get_recent_notes(
    ...
    notes = await NotesRepository.get_recent_notes(db, limit=limit)
